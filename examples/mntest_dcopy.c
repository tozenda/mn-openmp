...
int main (int argc, char **argv)
{
  ...
  int exp ;
  printf("Comparaison pour DCOPY entre CBLAS, notre fonction non parallélisée et notre fonction parallelisée\n");
 /* Calcul du residu de la mesure */
  ...
  for (exp = 0 ; exp < NBEXPERIMENTS; exp++)
    {
      ...
      start = _rdtsc () ;
         ...
         mncblas_dcopy_noomp (VECSIZE, vec1, 1, vec2, 1) ;
  ...
  printf ("cblas_dcopy_noomp : nombre de cycles: \t %Ld ;\t GFLOP/s :\t %3.3f\n ", av-residu,((((double) 2 * (double) VECSIZE * (double) VECSIZE)) / ((double) (av - residu) * (double) 0.38)));
  ...
  for (exp = 0 ; exp < NBEXPERIMENTS; exp++)
    {
      ...
      start = _rdtsc () ;
          ...
          mncblas_dcopy_omp (VECSIZE, vec1, 1, vec2, 1) ;
  ...
  // vector_print (vec2) ;
  printf ("cblas_dcopy_omp : nombre de cycles: \t %Ld ;\t GFLOP/s :\t %3.3f\n ", av-residu,((((double) 2 * (double) VECSIZE * (double) VECSIZE)) / ((double) (av - residu) * (double) 0.38)));
