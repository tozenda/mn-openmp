...
int main (int argc, char **argv)
{
 ...
 /* Calcul du residu de la mesure */
  ...
  printf ("cblas_scopy : nombre de cycles: \t %Ld \n", av-residu) ;
  ...
  for (exp = 0 ; exp < NBEXPERIMENTS; exp++)
    {
      ...
      start = _rdtsc () ;
         ...
         mncblas_scopy_noomp (VECSIZE, vec1, 1, vec2, 1) ;
  ...
  printf ("mncblas_scopy_noomp: nombre de cycles: \t %Ld \n", av-residu) ;
  ...
  for (exp = 0 ; exp < NBEXPERIMENTS; exp++)
    {
      ...
      start = _rdtsc () ;
          ...
          mncblas_scopy_omp (VECSIZE, vec1, 1, vec2, 1) ;
  ...
  // vector_print (vec2) ;
  printf ("mncblas_scopy_omp: nombre de cycles: \t %Ld \n", av-residu) ;
